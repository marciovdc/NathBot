...
const { nathmenu } = require('./src/nathmenu')
...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5541988576232:+55 (41) 98857-6232\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Perai fof to fazendo bb... ⌛',
				success: '✔️ Conseguiii ✔️',
                                levelon: '❬ ✔ ❭ *habilitar Level*',
				leveloff: ' ❬ X ❭  *desabilitar Level*',
				levelnoton: '❬ X ❭ *level não ativo*',
				levelnol: '*Pqp kskst level* 0 ',
				error: {
					stick: '[❗] Desculpa, não deu pra fazer tenta dnovo ou outra img❌',
					Iv: '❌ Link inválido burro ❌'
				},
				only: {
					group: '[❗] Comando só usado em grupos bb! ❌',
					ownerG: '[❗] Comando só usado pelo grupo proprietário! ❌',
					ownerB: '[❗] Comando só usado pela NaTh! Sai... ❌',
					admin: '[❗] Vc eh membro comum, sai daqui! ❌',
					Badmin: '[❗] Este comando só pode ser usado quando o bot se torna administrador! ❌',
                                        daftarB: `── 「REGISTRE-SE」 ──\nOlá como!\nVocê não está registrado no banco de dados, \n\nComando : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar Nath|18`,
				}
			}
			...
			const ownerNumber = ["5541988576232@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5541988576232@s.whatsapp.net'
                        /******ApiKey Input******/
...
case 'timer':
		...
		client.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
				break
				case 'nathmenu':
					client.sendMessage(from, nathmenu(prefix, sender), text, {quoted: mek})
							break
							case 'menuadmin':
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Tá, tirei o adm. Virou membro comum ksksksk!!! :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`Tá, tirei o adm. Virou membro comum ksksksk!!! @${mentioned[0].split('@')[0]}\n*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
                  ...
                  case 'promote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Blz, agora é um adm. Chupem membros comuns!!! :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Blz, agora é um adm. Chupem membros comuns!!! : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
                case 'creator':
                  client.sendMessage(from, {displayname: "Nath", vcard: vcard}, MessageType.contact, { quoted: mek})
               client.sendMessage(from, 'Aqui está o número do meu dono, manda nudes pra ele ksksksksks!!!',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
              ...
              await client.sendMessage(from, options, text)
               break
				...
				case 'snowwrite':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}snowwrite texto1|texto2, exemplo ${prefix}snowwrite Nath|BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}marvellogo texto, por exemplo ${prefix}marvellogo Nath BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'testime':
					...
					break
				case 'semoji':
					if (args.length < 1) return reply('E-E-Emoji?')
                                        if (!isUser) return reply(mess.only.daftarB)
					ranp = getRandom('.png')
					rano = getRandom('.webp')
	...
	case 'kbbi':
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'block':
					...
					client.sendMessage(from, `Pedidos recebidos, block ${body.slice(8)}@c.us`, text)
					break
				case 'hilih':
					client.updatePresence(from, Presence.composing) 
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 Chamando Todos 〙✪══\n╠➥'+teks+'╚═〘 NaTh BOT 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘 Chamando Todos 〙✪══\n╠➥'+teks+'╚═〘 NaTh BOT 〙', text, {quoted: mek})
					break
                case 'tagall3':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘 Chamando Todos 〙✪══\n╠➥'+teks+'╚═〘 NaTh BOT 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'ocr':
					...
					break
                     ...
                     const bug = body.slice(5)
                      if (pesan.length > 300) return client.sendMessage(from, 'Credo, muito texto, só no máximo de 300 letras. Palhaço!', msgType.text, {quoted: mek})
                    ...
                    reply('Problemas foram relatados ao proprietário do BOT, relatórios falsos serão ignorados.')
                    break
               case 'apakah':
               client.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
			...
			case 'closegc':
					...
					var nomor = mek.participant
					const close = {
					text: `Grupo fechado pelo administrador @${nomor.split("@s.whatsapp.net")[0]}\nagora *apenas os tops* podem enviar mensagens`,
					contextInfo: { mentionedJid: [nomor] }
					}
                ...
                case 'bukagc':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					open = {
					text: `Grupo aberto pelo administrador @${sender.split("@")[0]}\nagora *todos os membros comuns* podem enviar mensagens`,
					contextInfo: { mentionedJid: [sender] }
					}
                ...
                case 'ninjalogo':
                      if (args.length < 1) return reply('Cadê o texto burro?')
                      if (!isUser) return reply(mess.only.daftarB)
                      gh = body.slice(11)
                      gl1 = gh.split("|")[0];
                      gl2 = gh.split("|")[1];
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`onde está o texto? exemplo ${prefix}logowolf NaTh|BOT`)
					reply(mess.wait)
				...
				case 'logowolf2':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply(`Cadê o texto, hum\nExemplo: ${prefix}phlogo |NaTh|BOT`)
					reply(mess.wait)
					...
					break
                case 'truth':
					const trut =['Você já gostou de alguém? quanto tempo? ',' Se possível ou se você quiser, na gc / fora da gc com quem você vai fazer amizade? (você pode ser diferente / do mesmo sexo) ',' qual é o seu maior medo? ',' você já gostou alguém e sentiu essa pessoa como você também? ',' Qual é o nome do seu amigo ex-namorado que você gostava secretamente? ',' Você já roubou o dinheiro da sua mãe ou do pai? O motivo? ',' O que te faz feliz quando você está triste ',' você já foi um amor não correspondido? se alguma vez com quem? como você se sente, brou? ',' você já foi traído por outras pessoas? ',' a coisa mais temida ',' quem é a pessoa mais influente em sua vida ',' que orgulho você teve este ano ', 'quem é a pessoa que pode te deixar com tesão', 'quem é a pessoa que já te deixou com tesão', '(bgi que é muçulmano) nunca orou o dia todo?', 'Quem é o mais próximo do seu tipo de parceiro ideal aqui ',' quem gosta de brincar com quem? ',' já rejeitou as pessoas? o motivo por quê? ',' Mencione o incidente que te machucou e que você ainda lembra ',' o que você ganhou este ano? ',' Qual era o seu pior hábito na escola?']
					const ttrth = trut[Math.floor(Math.random() * trut.length)]
					truteh = await getBuffer(`https://i.ibb.co/305yt26/bf84f20635dedd5dde31e7e5b6983ae9.jpg`)
					client.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: mek })
					break
                                case 'dare':
					const dare =['Envie uma mensagem para o seu ex e diga "Eu ainda gosto de você",' ligue para a namorada / namorada agora e ss para o jogador ',' papai para um dos membros do grupo ',' Diga "VOCÊ É BONITO E NÃO FOI AMIGÁVEL" para pessoal ',' ss recente chamada whatsapp ',' drop emoticon "ðŸ¦„ ¨'¨ "cada digitação no gc / pc por 1 dia ',' enviar uma nota de voz dizendo posso te ligar, baby? ',' solte uma música / quote, continue As tags de membros são adequadas para a citação ',' use uma foto do Sule por 3 dias ',' digite no idioma local 24 horas ',' mude o nome para "Sou uma criança fofa, luna" por 5 horas ',' bate-papo com contatos wa o pedido de acordo com% sua bateria, continue dizendo a ele "tive sorte em ter você ',' bate-papo ex e diga" eu te amo, pgn de volta ',' gravar voz ler surah al-kautsar ',' disse "eu tenho uma queda por você, quer meu namorado ou não?" para o último sexo oposto que você conseguiu conversar (entregar em wa / tele), espere ele responder, se ss cair aqui ', 'diga o tipo de sua namorada!', 'tire / poste fotos de namorada / esmague', 'gritou gajelas e envie usando vn aqui', 'faça um papinho na cara e envie para um de seus amigos u ',' envie sua foto com uma legenda, sou um filho adotivo ',' gritou com palavras ásperas enquanto Vn então a enviou aqui ',' gritou "anjimm gabutt anjimmm" na frente de sua casa ',' mude o nome para "BOWO" por 24 horas ',' Fingir estar possuído, por exemplo: ser possuído por um maung, possuído por um gafanhoto, possuído por uma geladeira, etc. '
				]
					...
					case 'nsfwarmpits':
				        ...
				        uptime = process.uptime()
                                ...
                                case 'neonlogo':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}neonlogo NathBOT`)
                                        reply(mess.wait)
                                ...
                                case 'neonlogo2':
                                        ...
                                        teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto rum\nExemplo: ${prefix}neonlogo2 NaThBOT`)
                                        reply(mess.wait)
                                ...
                                case 'lionlogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`onde está o texto rum\nContoh: ${prefix}lionlogo NaTh|BOT`)
                                        reply(mess.wait)
                                ...
                                case 'jokerlogo':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto rum\nExemplo: ${prefix}jokerlogo NaThBOT`)
                                        reply(mess.wait)
			       ...
			       reply(anu.result.jadwalTV)
					break
                                ...
                                case 'burnpaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto rum\nExemplo: ${prefix}burnpaper NaThBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'lovepaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto rum\nExemplo: ${prefix}lovepaper NaThBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'woodblock':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto rum\nExemplo: ${prefix}woodblock NaThBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'qowheart':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto rum\nExemplo: ${prefix}qowheart NaThBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'mutgrass':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto rum\nExemplo: ${prefix}mutgrass NaThBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'undergocean':
                                        var gh = body.slice(12)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto rum\nExemplo: ${prefix}undergocean NaThBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'woodenboards':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto rum\nExemplo: ${prefix}woodenboards NaThBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'wolfmetal':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto rum\nExemplo: ${prefix}wolfmetal NaThBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'metalictglow':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto rum\nExemplo: ${prefix}metalictglow NaThBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'randomkpop':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aff...'})
                                        break
                                case 'fml2':
                                        ...
                                        break
                                case 'tiktok':
					if (args.length < 1) return reply('Onde está o link, rum?')
					...
					break
				case 'ttp':
					if (args.length < 1) return reply('rum')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				case 'clone':
					...
					break
                                ...
                                case 'pubglogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`onde está o texto rum\nExemplo: ${prefix}pubglogo NaTh|BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                ...
                                case 'harrypotter':
                                        var gh = body.slice(12)
                                        if (args.length < 1) return reply(`onde está o texto rum\nExemplo: ${prefix}harrypotter NaThBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
